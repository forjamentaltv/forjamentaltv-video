...
app.post('/generate-thumbnail', async (req, res) => {
  ...
  try {
    ...
    console.log(`Thumbnail: ${titulo} | foto: ${photoQuery}`);

    const titleLines = wrapText(normalizeForSvg(titulo).toUpperCase(), 18);
    const fontSize = titleLines.length > 2 ? 78 : 88;
    ...
    const titleSvg = titleLines.map(function(line, i) {
      ...
      return `<text x="${cx}" y="${y}" text-anchor="middle" font-family="Arial Black,Arial" font-size="${fontSize}" font-weight="900" fill="white" stroke="black" stroke-width="3" paint-order="stroke">${escapeXml(line)}</text>`;
    }).join('\n');
  ...
  <defs>
    <linearGradient id="grad" x1="0" y1="0" x2="0" y2="1">
      ...
      <stop offset="45%"  stop-color="#000" stop-opacity="0.50"/>
  ...
  <text x="${cx}" y="${H - 32}" text-anchor="middle" font-family="Arial Black,Arial" font-size="22" font-weight="bold" fill="#c9a84c" letter-spacing="6" opacity="0.9">FORJA MENTAL TV</text>
    ...
    if (photoUrl) {
      ...
      await sharp(bgBuf).composite([{ input: Buffer.from(svg), top: 0, left: 0 }]).jpeg({ quality: 92 }).toFile(outputPath);
      fs.existsSync(photoPath) && fs.unlinkSync(photoPath);
    } else {
      await sharp({ create: { width: W, height: H, channels: 3, background: { r: 10, g: 10, b: 10 } } })
        .composite([{ input: Buffer.from(svg), top: 0, left: 0 }]).jpeg({ quality: 92 }).toFile(outputPath);
    }
